...
enum Block : int { Battery, Memory, Date, MAX_BLOCKS };
    ...
    void * /* params */)
{
  for (;;) {
    ...
    sprintf(str,
            " ^c#c7d7e8^\uf017  ^c#bbbbbb^%s %s %d %s %.02d:%.02d ",
            days[date->tm_wday],
            months[date->tm_mon],
            date->tm_mday,
    ...
    void * /* params */)
{
  for (;;) {
    ...
    for (int i = 0; fgets(line, sizeof(line), file);) {
      if (i == 5) break;
      if ((sscanf(line, "MemTotal: %lu kB", &memTotal) == 1) || (sscanf(line, "MemFree: %lu kB", &memFree) == 1) ||
          (sscanf(line, "Buffers: %lu kB", &buffers) == 1) || (sscanf(line, "Cached: %lu kB", &cached) == 1) ||
